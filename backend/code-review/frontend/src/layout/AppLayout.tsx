/* layout/AppLayout.tsx — 三段式外壳:Sidebar + Header + 内容区(对照原型 app.jsx) */
import { useState } from 'react'
import { Layout, Menu, Input, Badge, Dropdown, Select } from 'antd'
import {
  AppstoreOutlined, ApartmentOutlined, SafetyOutlined,
  BarChartOutlined, MessageOutlined, BookOutlined, SettingOutlined, TeamOutlined,
  BellOutlined, SearchOutlined, MenuFoldOutlined, MenuUnfoldOutlined, ScanOutlined,
...
export default function AppLayout() {
  ...
  const projectItems = PROJECT_TABS.map((t) => ({
  ...
  const crumbProject = curProject?.name
  const crumbTab = PROJECT_TABS.find((t) => t.key === tab)?.label

  return (
    <Layout style={{ height: '100vh' }}>
      <Sider width={224} collapsedWidth={64} collapsed={collapsed} theme="light"
        style={{ borderRight: '1px solid var(--border-2)' }}>
        <div onClick={() => nav('/dashboard')}
          style={{ height: 56, display: 'flex', alignItems: 'center', gap: 10, padding: '0 16px', cursor: 'pointer' }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'grid', placeItems: 'center', color: '#fff' }}>
            <ScanOutlined />
          </div>
        ...
        {user?.role === 'admin' && <>
          ...
          <Menu mode="inline" selectedKeys={[selectedKey]}
            items={[{ key: '/users', icon: <TeamOutlined />, label: '用户管理' }]}
            onClick={(e) => nav(e.key)} style={{ border: 'none' }} />
        </>}
      </Sider>

      <Layout>
        <Header style={{ height: 56, display: 'flex', alignItems: 'center', padding: '0 20px', borderBottom: '1px solid var(--border-2)', gap: 16 }}>
          <span style={{ cursor: 'pointer', color: 'var(--text-3)' }} onClick={toggleCollapsed}>
            {collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />}
          </span>
          <div style={{ color: 'var(--text-2)', fontSize: 14 }}>
            <span style={{ cursor: 'pointer' }} onClick={() => nav('/dashboard')}>工作台</span>
            {inProject && crumbProject && <> › <span>{crumbProject}</span></>}
          ...
          <Dropdown menu={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <span style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--primary)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 12, fontWeight: 600 }}>
                {(user?.name || 'AD').slice(0, 2)}
              </span>
        ...
        <Content style={{ overflowY: 'auto' }}>
          <div className="fade-up" style={{ padding: 24, maxWidth: 1480, margin: '0 auto' }}>
            <Outlet />
          </div>
        </Content>
      </Layout>
