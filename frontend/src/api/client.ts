...
export async function getOne<T>(url: string, params?: any): Promise<T> {
  const r = await http.get(url, { params }); return r.data
}
export async function post<T>(url: string, body?: any): Promise<T> {
  const r = await http.post(url, body); return r.data
}
export async function put<T>(url: string, body?: any): Promise<T> {
  const r = await http.put(url, body); return r.data
}
export async function patch<T>(url: string, body?: any): Promise<T> {
  const r = await http.patch(url, body); return r.data
}
export async function del<T>(url: string): Promise<T> {
  const r = await http.delete(url); return r.data
}
